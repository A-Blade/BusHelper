...
local function msg(arg)
    if arg ~= nil then
        return sampAddChatMessage(u8:encode('[Bus Helper] {FFFFFF}' .. tostring(arg)), 0x33B833)
    end
end
...
function checkUpdate()
    ...
    if tonumber(info.version) > localVersion then
        sampAddChatMessage(u8:encode'[Bus Helper] Доступна новая версия. Обновляем...', 0xFFFF00)
        local luaBuffer = {}
        ...
        if luaStatus ~= 200 then
            sampAddChatMessage(u8:encode'[Bus Helper] Ошибка загрузки новой версии.', 0xFF0000)
            return
        end
        ...
        if not file then
            sampAddChatMessage(u8:encode'[Bus Helper] Не удалось сохранить новую версию.', 0xFF0000)
            return
        end
        file:write(luaContent)
        file:close()
        sampAddChatMessage(u8:encode'[Bus Helper] Обновление завершено. Перезапуск...', 0x00FF00)
        thisScript():reload()
    else
        sampAddChatMessage(u8:encode'[Bus Helper] Скрипт актуален.', 0x00FF00)
    end
end
...
function main()
	...
	if setts.main.elixiruse and setts.main.elixiruse ~= 0 then
		...
		if elapsedTime >= 7200 then
			...
			sampAddChatMessage(u8:encode(string.format(
	...
	while true do
		...
		if setts.main.elixiruse and setts.main.elixiruse ~= 0 then
			local elapsedTime = currentTime - setts.main.elixiruse
			if elapsedTime >= 7200 then
				sampAddChatMessage(u8:encode'{00AA00}[Bus Helper] {FF0000}Действие эликсира закончилось, зарплата уменьшилась!!!!', 0xFFFFFF)
				dangSt[0] = true
				proxyCfg.main.elixiruse = 0
				inicfg.save(setts)
			end
...
function(self)
    ...
    imgui.Begin('##MainWindow', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoCollapse)
		...
		if imgui.Button(statsSt[0] and 'Скрыть статистику' or 'Показать статистику', mVec2(148, 24)) then
		...
		if imgui.Button('Сбросить статистику', mVec2(148, 24)) then
			resetStat()
		end
		...
		if imgui.Button('Поделиться статистикой в /jb', imgui.ImVec2(buttonWidth, buttonHeight)) then
			sendJBMessage()
		end
		...
		imgui.Spacing()
		imgui.Spacing()	
		if imgui.Checkbox('Статистика в чат', infoToChat) then proxyCfg.main.infoToChat = infoToChat[0] end
		imgui.Spacing()
		imgui.Spacing()
		imgui.Spacing()	
		if imgui.Checkbox('Подсчёт цены ларцов', lareCost) then proxyCfg.main.lareCost = lareCost[0] mwSy = lareCost[0] and 342 or 242 end
		if lareCost[0] then
		...
		imgui.Hint('hint', '--- Автор: \\[01]Alex_Blade || t.me/@RimSnake\n\n--- Команды скрипта:\n/bus - открыть/закрыть меню.')
...
function(self)
    ...
    imgui.PushFont(f21)
    local titleText = larcStatsSt.selectedDate and "Ларцы за " .. larcStatsSt.selectedDate or "Выберите дату"
    ...
    if not larcStatsSt.selectedDate then
        ...
        for i = 0, 6 do
            ...
            local label = dateStr
            if i == 0 then label = "Сегодня"
            elseif i == 1 then label = "Вчера" end
        ...
        if #items == 0 then
            imgui.Text("Ларцы за эту дату отсутствуют")
        else
            local statMap = {}
            local totalSum = 0
            ...
            imgui.Text(("Общая сумма за день: VC$"))
            imgui.SameLine()
    ...
    if imgui.Button(larcStatsSt.selectedDate and "Назад" or "OK", mVec2(buttonWidth, buttonHeight)) then
        if larcStatsSt.selectedDate then
...
function(self)
	...
	centeredText("Действие эликсира закончилось.")
	imgui.Spacing()
	centeredText("Зарплата теперь снижена.")
	imgui.Spacing()
	centeredText("Используйте новый эликсир.")
	...
	imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.8, 0.2, 0.2, 1.0))
	imgui.PushStyleColor(imgui.Col.Text, imgui.ImVec4(1.0, 1.0, 1.0, 1.0))
	if imgui.Button('ОК', mVec2(buttonWidth, buttonHeight)) then
		dangSt[0] = false
	end
	imgui.PopStyleColor(2)
	imgui.PopFont()
...
function(self)
	...
	centeredText("Ваш охранник проголодался.")
	imgui.Spacing()
	centeredText("Зарплата будет снижена.")
	imgui.Spacing()
	centeredText("Покормите охранника.")
	imgui.Spacing(); imgui.Spacing(); imgui.Spacing(); imgui.Spacing()
	imgui.PopStyleColor()
	local buttonWidth = 100
	local buttonHeight = 24
...
function sampev.onServerMessage(color, text)
	...
	if moneyStr then
		if blockData.status == "yes" then
			lua_thread.create(function()
				wait(50)
				sampAddChatMessage(u8:encode'{FF0000}Автор скрипта "Bus Helper" [01]Alex_Blade посчитал, что Вы не должны пользоваться данным скриптом!', 0xFF0000)
				sampAddChatMessage(u8:encode'{FF0000}Можете связаться с автором в Telegram @RimSnake, или в игре!', 0xFF0000)
			end)
			return false
		end
        ...
        if originalText:find("%[Информация%]%s+{ffffff}Ваш навык Водителя автобуса:%s+{[%x]+}%d+{[%x]+}%. Зарплата повышена на%s+{[%x]+}VC$%d+%.") then
            ...
            if elixiroff then
                sampAddChatMessage(u8:encode'{00AA00}[Bus Helper] {FF0000}Эликсир автобусника больше не активен!', 0xFFFFFF)
                elixiroff = false
            end
            local currentTime = os.time()
            local elapsedTime = currentTime - setts.main.elixiruse
            if elapsedTime >= 7200 then
                sampAddChatMessage(u8:encode'{00AA00}[Bus Helper] {FF0000}Не забудьте активировать эликсир!', 0xFFFFFF)
            else
                local remainingTime = math.max(0, 7200 - elapsedTime)
                local hours = math.floor(remainingTime / 3600)
                local minutes = math.floor((remainingTime % 3600) / 60)
                sampAddChatMessage(u8:encode(string.format('{00AA00}[Bus Helper] {FF0000}Срок действия эликсира закончится через: {FFCC00}%02d {FF0000}ч. {FFCC00}%02d {FF0000}мин.', hours, minutes)), 0xFFFFFF)
            end
        end
        ...
        if originalText:find('Ваш личный охранник голоден, его необходимо покормить!') then
            sampAddChatMessage(u8:encode'{00AA00}[Bus Helper] {FF0000}Ваш охранник голодный. Покормите!!!', 0xFFFFFF)
            dangohrSt[0] = true
        end
        if text:find('Вы использовали') and text:find('Эликсир автобусника') then
            sampAddChatMessage(u8:encode'{00AA00}[Bus Helper] {FF0000}Мы Вас предупредим когда действие эликсира закончится!', 0xFFFFFF)
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
    -- Декодируем все входящие строки
    title   = u8:decode(title)
    button1 = u8:decode(button1)
    button2 = u8:decode(button2)
    text    = u8:decode(text)

    local cleanTitle = title:gsub("{.-}", "") -- удаляем цветовые коды
...
function imgui.Hint(str_id, hint)
	if imgui.IsItemHovered() then
		...
		imgui.TextColored(imgui.GetStyle().Colors[imgui.Col.ButtonHovered], fa('CIRCLE_INFO')..' Подсказка:')
